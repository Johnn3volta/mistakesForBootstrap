    ...
    function send($mailTo, $subject, $message, $headers) {
        ...
        try {
            ...
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new Exception('Ошибка отправки команды: HELO');
            }
            ...
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new Exception('Ошибка отправки команды: MAIL FROM');
            }
    
         // если получатель 1, то эта //
//            $mailTo = ltrim($mailTo, '<');
//            $mailTo = rtrim($mailTo, '>');
//            fputs($socket, "RCPT TO: <" . $mailTo . ">\r\n");
//            if (!$this->_parseServer($socket, "250")) {
//                fclose($socket);
//                throw new Exception('Ошибка отправки команды: RCPT TO');
//            }
    
            // если получателей более 1, то эта //
            ...
            if (!$this->_parseServer($socket, "354")) {
                fclose($socket);
                throw new Exception('Ошибка отправки команды: DATA');
            }
            
            fputs($socket, $contentMail."\r\n.\r\n");
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new Exception("E-mail не оправлен");
            }
